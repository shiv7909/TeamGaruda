...
class _AnalyticsState extends State<Analytics> {
            ...
            LayoutBuilder(builder: (context, constraints) {
              ...
              return Column(
                children: [
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    _FilterDropdown(
                                      ...
                                      onChanged: (value) {
                                        setState(() {
                                          selectedPeriod = value ?? 'Last 30 days';
                                        });
                                      },
                                    ),
                                  ],
                                ...
                                const SizedBox(height: 24),
                                SizedBox(height: 300, child: _IssuesBarChart(issues: issues)),
                              ],
                            ),
                          ),
                        ),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 24),
                                SizedBox(height: 300, child: _IssuesDonutChart(issues: issues)),
                              ],
                            ),
                          ),
                        ),
                  ...
                  // Detailed Analytics
                  _buildDetailedAnalytics(constraints.maxWidth < 600),
                  
                  const SizedBox(height: 24),
                  
                  // Real-time Trends
                  _buildRealTimeTrends(),
                ],
              );
            }),
          ],
  ...
  Widget _buildKeyMetrics() {
    ...
    final resolutionRate = issues.isEmpty ? 0 : 
        (issues.where((i) => i.status.toLowerCase() == 'resolved').length / issues.length * 100);
    final avgResponseTime = _calculateAverageResponseTime();
    final satisfactionScore = 87; // This would come from user feedback
  ...
  Widget _buildDetailedAnalytics(bool isMobile) {
    if (isMobile) {
      return Column(
        children: [
          _buildResolutionTimesCard(),
      ...
      return Row(
        children: [
          Expanded(child: _buildResolutionTimesCard()),
          const SizedBox(width: 24),
          Expanded(child: _buildGeographicCard()),
        ],
      );
    }
  }

  Widget _buildResolutionTimesCard() {
    final categoryTimes = _calculateResolutionTimesByCategory();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ...categoryTimes.entries.map((entry) => _ResolutionTimeCard(
              category: entry.key,
              avgTime: '${entry.value.toStringAsFixed(1)} days',
              ...
              child: Column(
                children: [
                  Row(
                    ...
                    children: [
                      Text('Issues with Location Data'),
                      Text('${locationStats['withLocation']!.toInt()}/${locationStats['total']!.toInt()}',
                           style: const TextStyle(fontWeight: FontWeight.w600)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  LinearProgressIndicator(
                    value: locationStats['percentage']! / 100,
                    backgroundColor: Colors.grey.shade300,
                    valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFF10B981)),
                  ),
                  const SizedBox(height: 8),
                  Text('${locationStats['percentage']!.toStringAsFixed(1)}% completion rate',
                       style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                ],
              ),
            ),
            const SizedBox(height: 16),
            const Text('Location Heatmap', style: TextStyle(fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            Container(
              height: 150,
              decoration: BoxDecoration(
              ...
              child: const Center(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8),
                    Text('Interactive Map', style: TextStyle(fontWeight: FontWeight.w600)),
                    Text('Geographic distribution', style: TextStyle(fontSize: 12)),
                  ],
                ),
              ),
            ),
                ...
                Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 4),
                      Text('Live', style: TextStyle(fontSize: 11, color: Color(0xFF10B981))),
                    ],
                  ),
                ),
              ],
            ...
            Wrap(
              ...
              children: [
                ...
                _TrendCard(
                  ...
                  value: '${_calculateAverageResponseTime()}h',
                  change: '-8%',
                  isPositive: true,
                  icon: Icons.timer,
                ),
  ...
  int _calculateAverageResponseTime() {
    ...
    final resolved = issues.where((i) => i.status.toLowerCase() == 'resolved');
    if (resolved.isEmpty) return 0;
    
    final totalHours = resolved.map((issue) {
      final now = DateTime.now();
      return now.difference(issue.createdAt).inHours;
    }).reduce((a, b) => a + b);
    
    return (totalHours / resolved.length).round();
  }

  Map<String, double> _calculateResolutionTimesByCategory() {
    final Map<String, List<int>> categoryTimes = {};
    
    for (final issue in issues.where((i) => i.status.toLowerCase() == 'resolved')) {
      final days = DateTime.now().difference(issue.createdAt).inDays.toDouble();
      categoryTimes.putIfAbsent(issue.issueType, () => []).add(days.toInt());
    }
    
    return categoryTimes.map((category, times) {
      final avg = times.isEmpty ? 0.0 : times.reduce((a, b) => a + b) / times.length;
      return MapEntry(category, avg);
    });
  }

  Map<String, double> _calculateLocationStats() {
    final total = issues.length;
    final withLocation = issues.where((i) => i.latitude != null && i.longitude != null).length;
    final percentage = total == 0 ? 0.0 : (withLocation / total) * 100;
    
    return {
      'total': total.toDouble(),
      'withLocation': withLocation.toDouble(),
      'percentage': percentage,
    };
  }

  int _getTodayReports() {
    final today = DateTime.now();
    return issues.where((issue) {
      final issueDate = issue.createdAt;
      return issueDate.year == today.year &&
             issueDate.month == today.month &&
             issueDate.day == today.day;
    }).length;
  }
...
class _IssueActions extends StatelessWidget {
  ...
  void _openInMaps(double latitude, double longitude, BuildContext context) {
    // This would typically open the system maps app
    // For now, we'll show a snackbar with coordinates
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          ...
          onPressed: () {
            ...
            print('Coordinates copied: $latitude, $longitude');
          },
        ),
      ),
    );
  ...
  Future<void> _loadData() async {
    try {
      final [usersData, analyticsData] = await Future.wait([
        SupabaseService.getUsers(status: selectedStatus),
        SupabaseService.getDashboardAnalytics(),
      ]);
        ...
        if (isLoading)
          ...
          Wrap(
            ...
            children: [
              _StatCard(
                title: 'Total Users',
                value: '${analytics!.activeUsers + analytics!.bannedUsers}',
                icon: Icons.people,
                color: const Color(0xFF8B5CF6),
                trend: '+23',
                trendUp: true,
          ...
          const Center(child: Text('Failed to load user statistics')),
        const SizedBox(height: 32),
        // User Actions
        Card(
          elevation: 0,
              ...
              builder: (context, constraints) {
                ...
                return Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    _UserTable(users: users, isMobile: isMobile, isLoading: isLoading),
                  ],
                );
              },
            ),
...
class _IssuesBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BarChart(BarChartData(
      maxY: categoryData.values.isEmpty ? 10 : categoryData.values.reduce((a, b) => a > b ? a : b).toDouble() + 5,
  ...
  List<BarChartGroupData> _getBarGroups() {
    ...
    return categories.asMap().entries.map((entry) {
      ...
      return _makeGroupData(index, count.toDouble(), _getCategoryColor(category));
    }).toList();
  }
  ...
  Widget _getCategoryTitles(double value, TitleMeta meta) {
    ...
    if (value.toInt() < categories.length) {
      ...
      return SideTitleWidget(axisSide: meta.axisSide, space: 16, child: Text(shortName, style: style));
    }
  ...
  String _getShortCategoryName(String category) {
    switch (category.toLowerCase()) {
      case 'road damage': return 'Roads';
      case 'street lighting': return 'Lighting';
      case 'water leaks': return 'Water';
      case 'waste management': return 'Waste';
      case 'safety issues': return 'Safety';
      default: return category.split(' ').first;
    }
  }

  Color _getCategoryColor(String category) {
    switch (category.toLowerCase()) {
      case 'road damage': return const Color(0xFF3B82F6);
      case 'water leaks': return const Color(0xFF06B6D4);
      case 'street lighting': return const Color(0xFFF59E0B);
      case 'waste management': return const Color(0xFF10B981);
      case 'safety issues': return const Color(0xFFEF4444);
...
class _IssuesDonutChart extends StatelessWidget {
  ...
  const _IssuesDonutChart({required this.issues});
    ...
    if (statusData.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.donut_large, size: 48, color: Colors.grey.shade400),
            const SizedBox(height: 16),
            Text('No data available', style: TextStyle(color: Colors.grey.shade600)),
          ],
        ),
        ...
        Positioned(
          ...
          child: Column(
            children: statusData.entries.map((entry) => 
              _buildLegendItem(entry.key, _getStatusColor(entry.key), '${(entry.value / issues.length * 100).toStringAsFixed(1)}%')
            ).toList(),
          ),
        ),
      ],
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: value,
          hint: Text(label, style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
